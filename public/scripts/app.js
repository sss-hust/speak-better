    ...
    function addMessage(msg) {
      if (!msg.id) msg.id = `msg-${Date.now()}-${messageIdSeed++}`;
      ...
      wrap.innerHTML = `
        ...
        <div class="msg-body">
          <div class="meta">${msg.me ? '我' : escapeHtml(msg.name || '')}</div>
          <div class="bubble">${escapeHtml(msg.text)}</div>
        </div>
      `;
      wrap.addEventListener('contextmenu', (e) => openContextMenu(e, msg));
      wrap.addEventListener('touchstart', (e) => {
        wrap._pressTimer = setTimeout(() => openContextMenu(e, msg), 550);
      });
      wrap.addEventListener('touchend', () => clearTimeout(wrap._pressTimer));
      wrap.addEventListener('touchmove', () => clearTimeout(wrap._pressTimer));
      wrap.addEventListener('touchcancel', () => clearTimeout(wrap._pressTimer));
    ...
    function browserHistoryState(view = currentView) {
      return {
        speakBetterView: view,
        chatId: currentChatId,
        depth: appHistoryDepth
      };
    }

    function rememberBrowserView(view, mode = 'push') {
      if (!window.history || syncingBrowserHistory) return;
      const url = window.location.href;
      if (mode === 'replace') {
        window.history.replaceState(browserHistoryState(view), '', url);
        return;
      }
      appHistoryDepth += 1;
      window.history.pushState(browserHistoryState(view), '', url);
    }

    function goBackInsideApp(fallbackView) {
      closeAllFloating();
      if (appHistoryDepth > 0) {
        window.history.back();
        return;
      }
      switchView(fallbackView, { historyMode: 'replace' });
    }
    ...
    function showPlaceholderToast(message = '鹅目前只负责帮你说话，别的功能……就等鹅下次展示！', event = null) {
      ...
      const sourceRect = event && event.currentTarget ? event.currentTarget.getBoundingClientRect() : appRect;
      const rawX = event && Number.isFinite(event.clientX) ? event.clientX : sourceRect.left + sourceRect.width / 2;
    ...
    function clearChatVoiceStatus() {
      const status = document.getElementById('chatVoiceStatus');
      if (!status) return;
      status.textContent = '';
      status.classList.remove('show', 'error', 'ok');
    }
    ...
    function closeEmojiPanel() {
      ...
      document.getElementById('app').classList.remove('emoji-mode');
    }
    ...
    function startPreviewSpeech(buttonId, inputId, statusId) {
      startSpeechToText({
        ...
        inputId,
        statusId,
        idleText: '语音识别完成，可以继续编辑或点「会说」。',
        emptyText: '还没有听到有效语音。请允许麦克风后说一句，或直接在输入框打字。',
        onFinal: (text) => {
          const status = document.getElementById(statusId);
          if (String(text || '').trim()) {
            setVoiceStatus(status, '语音识别完成，可以继续编辑或点「会说」。', 'ok');
          }
        }
      });
    }
    ...
    window.addEventListener('popstate', (event) => {
      ...
      appHistoryDepth = Math.max(0, state.depth || 0);
      if (state.chatId && state.chatId !== currentChatId) renderChatThread(state.chatId);
      switchView(state.speakBetterView, { historyMode: null });
      syncingBrowserHistory = false;
    });

    document.querySelectorAll('.tab').forEach(tab => {
      tab.addEventListener('click', () => {
        if (tab.classList.contains('placeholder-tab')) return;
    ...
    document.querySelectorAll('.placeholder-control,.placeholder-row,.placeholder-tab,.placeholder-tool').forEach(el => {
      el.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    function switchView(view, options = {}) {
      ...
      closeAllFloating();
      if (historyMode) rememberBrowserView(view, historyMode);
    }
    document.getElementById('cancelSpace').addEventListener('click', () => goBackInsideApp('qzoneTalk'));
    ...
    document.getElementById('chatVoice').addEventListener('click', () => {
      ...
      startSpeechToText({
        ...
        idleText: '识别完成，可以继续编辑或直接发送。',
        emptyText: '这次没有听到有效语音。可以再点一次，或直接打字。',
    ...
    document.getElementById('chatEmoji').addEventListener('click', () => {
      ...
      document.getElementById('chatEmoji').classList.toggle('active', emojiPanel.classList.contains('show'));
      document.getElementById('app').classList.toggle('emoji-mode', emojiPanel.classList.contains('show'));
      hideShade();
      if (emojiPanel.classList.contains('show')) chatList.scrollTop = chatList.scrollHeight;
    });
    emojiPanel.addEventListener('click', (event) => {
      const button = event.target.closest('[data-emoji]');
      if (!button) return;
      insertAtCursor(chatInput, button.dataset.emoji);
    });
    ...
    function startSpeechToText({ buttonId, inputId, statusId, idleText, emptyText, mergeWithExisting = false, onFinal }) {
      const status = document.getElementById(statusId);
      const input = document.getElementById(inputId);
      ...
      recognition.onresult = (event) => {
        ...
        input.value = mergeWithExisting
          ? `${initialValue.slice(0, selectionStart)}${text}${initialValue.slice(selectionEnd)}`
          : text;
      ...
      recognition.onend = () => {
        ...
        if (recognizedText) {
          setVoiceStatus(status, idleText, 'ok');
        } else if (!hadSpeechError) {
          setVoiceStatus(status, emptyText || '这次没有识别到语音。可以再点一次麦克风，或直接打字。', 'error');
        }
      };
      try {
        recognition.start();
    ...
    function parseVoiceIntent(text) {
      ...
      else if (/男朋友|女朋友|对象|恋人|男友|女友/.test(text)) target.value = '男朋友/女朋友';
      else if (/家人|亲人|爸|妈|父母|哥哥|姐姐|弟弟|妹妹/.test(text)) target.value = '亲人';
    ...
    function generateSay() {
      ...
      if (target === '男朋友/女朋友') {
        return pickBatch([
          ['我想认真跟你说一下我的感受，不是要吵架，是希望我们能好好理解彼此。', '我有点在意这件事，但更想和你好好说清楚。', '我不是想让你立刻给答案，只是希望你能知道我真实的想法。'],
          ['我嘴上可能有点别扭，但这件事我确实挺在意的。', '有点想你，也有点想让你主动一点点。', '我不是生气啦，就是希望你能多懂我一点。'],
          ['这件事我想真诚一点说：我希望我们可以一起把它聊开。', '我很珍惜我们，所以才想把话说得清楚一点。', '我想要的不是输赢，是我们能更舒服地相处。']
        ]);
      }
      if (target === '亲人') {
        return pickBatch([
          ['我知道你是为我好，我会认真考虑，也想按自己的节奏慢慢来。', '我最近确实有点累，但你不用太担心，我会照顾好自己。', '这件事我想和你商量一下，也希望你能听听我的想法。'],
          ['我明白你的担心，先让我自己试试看，可以吗？', '我会注意的，也谢谢你一直惦记我。', '有些话我可能说得不够好，但我知道你是关心我。'],
          ['放心啦，我会认真处理，不会随便糊弄自己的。', '我想先按自己的计划走一走，之后有情况会跟你说。', '谢谢你提醒我，我会放在心上的。']
        ]);
      }
      if (/真诚/.test(tones)) {
        return pickBatch([
          ['我想真诚地说一下我的想法，这件事对我来说还挺重要的。', '我不想把话说得太绕，所以直接一点：我希望我们可以认真聊聊。', '我理解你的立场，也想让你知道我的真实感受。'],
    ...
    // 长按消息菜单与帮我回
    // =========================
    function renderContextMenu(msg) {
      const ownActions = [
        ['copy', '▣', '复制'],
        ['forward', '↗', '转发'],
        ['favorite', '★', '收藏'],
    ...
    function openContextMenu(e, msg) {
      ...
      renderContextMenu(msg);
      const menu = document.getElementById('ctxMenu');
      const rect = document.getElementById('app').getBoundingClientRect();
      const point = e.touches && e.touches[0] ? e.touches[0] : e;
      const rawX = (point.clientX || 80) - rect.left;
      const rawY = (point.clientY || 200) - rect.top;
      const x = Math.max(14, Math.min(rect.width - 320, rawX - 160));
      const y = Math.max(60, Math.min(rect.height - 196, rawY + 12));
    ...
    function showMenuToast(text) {
      showPlaceholderToast(text, { clientX: 300, clientY: 520, currentTarget: document.getElementById('ctxMenu') });
    }

    function recallSelectedMessage() {
      if (!selectedReplySource || !selectedReplySource.me) return;
      const thread = chatThreads[currentChatId] || chatThreads.pcg;
      const index = thread.messages.findIndex(msg => msg.id === selectedReplySource.id);
      if (index >= 0) thread.messages.splice(index, 1);
      const node = document.querySelector(`.msg[data-message-id="${selectedReplySource.id}"]`);
      if (node) node.remove();
      document.getElementById('ctxMenu').classList.remove('show');
      showMenuToast('已模拟撤回这条消息。');
    }

    async function copySelectedMessage() {
      const text = selectedOriginalMsg || '';
      try {
        if (navigator.clipboard && navigator.clipboard.writeText) await navigator.clipboard.writeText(text);
        showMenuToast('已复制消息内容。');
      } catch (error) {
        showMenuToast('浏览器不允许直接复制，消息内容已选中在菜单里。');
      }
      document.getElementById('ctxMenu').classList.remove('show');
    }
    ...
    document.getElementById('ctxMenu').addEventListener('click', (e) => {
      ...
      if (action === 'helpReply') openReplyModalFromContext();
        ...
        document.getElementById('ctxMenu').classList.remove('show');
        showMenuToast(`${button.textContent.trim()}已模拟完成。`);
      }
    });
    ...
    // QQ 空间润色
    // =========================
    function getSpaceDraft() {
      return document.getElementById('spaceDraftPreview').value.trim()
        || document.getElementById('spaceVoiceIntent').value.trim()
        || document.getElementById('spaceText').value.trim()
        || '复习好累';
    }

    function updateSpaceDraftPreview() {
      const preview = document.getElementById('spaceDraftPreview');
      if (!preview) return;
